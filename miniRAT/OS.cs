...
namespace miniRAT
{
    ...
    {
        /// <summary>
        /// Return windows version Service pack and cpu info
        /// </summary>
        /// <returns></returns>
        public static string gtOSversionInfo()
        {
            StringBuilder sb = new StringBuilder(String.Empty);

            sb.Append(OS.Name + " ");
            sb.Append(OS.Edition);
            if (OS.ServicePack != string.Empty)
                sb.Append(String.Format(" SP:{0} ", OS.ServicePack));
            else
                sb.Append(" SP:None ");
            sb.Append(String.Format(" Version:{0} ", OS.VersionString));
            sb.Append(String.Format(" OS:{0} ", OS.OSBits));
            //sb.Append(String.Format("CPU:{0} ", OS.ProcessorBits));
            //sb.Append(String.Format("ProgramBits = {0}", OSVersionInfo.ProgramBits));
